...
4. **Frontend** — Next.js App Router in [frontend/src/app/](frontend/src/app/). `page.js` is the portfolio (light theme by default); `avatar/page.js` is the chat UI (forces dark) that POSTs to `${NEXT_PUBLIC_API_URL}/chat`. Hosted on Amplify (SSR / `WEB_COMPUTE`) and served at the custom domain `msalmansaeedch.de` (apex + `www`).
...
- **`frontend/public/twin-cover.png` is generated too, but its source is not in this repo.** It is served as `og:image` from [frontend/src/app/layout.js](frontend/src/app/layout.js), so a stale cover appears on every shared link. The HTML source and its renderer are kept outside version control (marketing assets, see `.gitignore`); ask the author to regenerate rather than hand-editing the PNG.
